...
class Publisher:
    ...
    def publish_vehicle_data(self, dataset_file_path: str) -> int:
        """
        Gets data from all vehicles in given dataset and publishes all vehicle data to data pipeline
        Returns number of succesful API calls which were published
        """
        # get all vehicle ids from dataset
        with open(dataset_file_path, mode="r") as file:
            data = json.load(file)

        # used to keep track of how many API requests were successful/published
        published = 0
        ...
        for vehicle_id in data["vehicle_ids"]:
            ...
            # immediately publish received data to the data pipeline
            if response_data:
                published += 1
                self.publish_data(published, response_data)
    ...
    def get_vehicle_data(self, vehicle_id: int) -> dict | None:
        ...
        Returns a dict of data specific to a vehicle if call is successful, otherwise None
        """
    ...
    def publish_data(self, count: int, data: dict):
        ...
        if count % 1000:
            print(f"Published {count} messages so far to {self.topic_path}")


if __name__ == "__main__":
    team_102_pub = Publisher(PROJECT_ID, TOPIC_ID)
    team_102_pub.publish_vehicle_data(DATASET_FILE_PATH)
