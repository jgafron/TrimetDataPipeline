...
TOPIC_ID = "bus-data"
#BUS_DATA_URL = "https://busdata.cs.pdx.edu/api/getStopEvents"
...
class Publisher:
    ...
    def publish_vehicle_data(self, dataset_file_path: str = DATASET_FILE_PATH, bs4_parse=False) -> tuple[int, int]:
        """
        ...
        for vehicle_id in data["vehicle_ids"]:
            response_data = self.get_vehicle_data(vehicle_id, bs4_parse)
            if not response_data:
                continue
            success += 1

            if bs4_parse:
                soup = BeautifulSoup(response_data.read(), "html.parser")
            ...
            for message in response_data:
                #print(message)
                published += 1
                self.publish_data(published, message)
            break

        return success, published


    def old_publish_vehicle_data(self, dataset_file_path: str) -> tuple[int, int]:
        """
        Gets data from all vehicles in given dataset and publishes all vehicle data to data pipeline
        Returns number of succesful API calls and numbers of messages published
        """
        ...
        for vehicle_id in data["vehicle_ids"]:
            ...
            if response_data:
                ...
                for r_data in response_data:
                    print(r_data)
                    published += 1
                    self.publish_data(published, r_data)

        return success, published

    def get_vehicle_data(self, vehicle_id: int, bs4_parse) -> list | None:
        """
        ...
        params = {"vehicle_num": vehicle_id} if bs4_parse else {"vehicle_id": vehicle_id}  # parameters (just vehicle_id in this case)
